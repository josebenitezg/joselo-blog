...
Therefore, we must rely on older (but robust) architectures like CNNs (Convolutional Neural Networks). Even considering the training of these advanced models is impractical for our purposes.

Even with advanced architectures like Vision Transformers, processing high-resolution images in real time becomes very slow. Yann LeCun has discussed this issue, emphasizing the need to understand our current technological standing and what is feasible with today's capabilities.
...
action detection and understanding people's behaviors around the shelves (which is incredibly difficult) and 
